...
export const CommentsAll: React.FC = () => {
    ...
    return (
        <>
            {commentAll.filter(comments => comments.user._id === localStorage.getItem('id')).map(comments => (
                <div key={comments._id} className="comments">
                    ...
                    <textarea onChange={(e) => setText(e.target.value)} className="comments__textarea">
                        {comments.text}
                    </textarea>
                    <div className="editing">
                        <button onClick={() => dispatch(patchComment(text, comments._id))} className="editing__btnEditing">Изменить</button>
                        <button onClick={() => dispatch(deleteComment(comments._id))} className="editing__btnDeleted">Удалить</button>
                    </div>
                </div>
            ))}
        </>
