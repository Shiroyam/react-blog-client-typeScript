...
export const PostsProfile: React.FC = () => {
    ...
    return (
        <>
            {(postItems ?? []).filter(post => post.user._id === localStorage.getItem("id")).map((post) => (
                <div
                    key={post._id}
                    className="posts"
                >
                    <div className="posts__textContainer">
                        ...
                        <div className="editing">
                            <Link className="editing__btnEditing" to={`/editingPage/${post._id}`}>Изменить</Link>
                            <button onClick={() => dispatch(deletePost(post._id))} className="editing__btnDeleted">Удалить</button>
                        </div>
                    </div>
